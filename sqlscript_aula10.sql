...
select titulo, au_snome as Autor from titulos, autores;

select titulo, au_snome as Autor from titulos, autores, tit_aut;

select titulo, au_snome as Autor from titulos, autores, tit_aut 
  where 
    tit_aut.au_id = autores.au_id;

select titulo, au_snome as Autor from titulos, autores, tit_aut 
  where 
    tit_aut.au_id = autores.au_id and 
    tit_aut.tit_id = titutlos.tit_id;

select titulo, au_snome as Autor from titulos, autores, tit_aut 
  where 
    tit_aut.au_id = autores.au_id and 
    tit_aut.tit_id = titulos.tit_id;
...
insert into autores (au_pnome, au_snome, cidade) values ('Raymond', 'Smullyan', 'Princeton');

update tit_aut 
  set 
    au_id = (select au_id from autores where au_snome = 'Smullyan') 
  where 
    tit_id = (select tit_id from titulos where upper(titulo) like '%BOOK%');
...
DELETE FROM AUTORES WHERE AU_ID = 235430498151931570511721067602871193404 
                       OR AU_ID = 235431951348895873813661274910258516248;

SELECT * FROM AUTORES;

select au_id, au_snome from autores where au_snome = 'Smullyan';

select count(au_snome) from autores where au_snome = 'Smullyan';

delete from autores  
  where au_id in (select au_id from autores where au_snome = 'Smullyan' limit 1);
...
select au_id from autores where au_snome = 'Smullyan' limit 1;

select au_id from autores where au_snome = 'Smullyan';

select au_id from autores where au_snome = 'Smullyan';

select au_id from autores where au_snome = 'Smullyan';

delete from autores where au_id = 235434274117507540001634806041018017780;

select * from autores;

select au_snome from autores, titulos, tit_aut where autores.au_id = tit_aut.au_id and 
  tit_aut.tit_id = titulos.tit_id and  titulos.titulo like 'What%';

update tit_aut 
  set au_id = (select au_id from autores where au_snome = 'Smullyan') 
  where 
    tit_id = (select tit_id from titulos, tit_aut where  
  tit_aut.tit_id = titulos.tit_id and  titulos.titulo like 'What%');

update tit_aut 
  set au_id = (select au_id from autores where au_snome = 'Smullyan') 
  where 
    tit_id = (select titulos.tit_id from titulos, tit_aut where  
  tit_aut.tit_id = titulos.tit_id and  titulos.titulo like 'What%');
